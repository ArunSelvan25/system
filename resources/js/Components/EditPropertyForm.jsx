...
export default function EditPropertyForm({ property, userId, propertyTypes, propertyStatuses, paymentFrequencies, formSubmitted }) {
    ...
    const { data, setData, processing, errors, reset } = useForm({
        ...
        security_deposit: property?.financials?.security_deposit || '',
        maintenance_charges: property?.financials?.maintenance_charges || '',
        lease_duration: property?.financials?.lease_duration || '',
        rent_due_day: property?.financials?.rent_due_day || '',
        is_negotiable: property?.financials?.is_negotiable ? 'true' : 'false',
        payment_frequency: property?.financials?.payment_frequency || '',
        ...
        status: (property?.status ? (property?.status == 1 ? 1 : 0) : 0)  || 0,
    });
    ...
    useEffect(() => {
        if (property?.images && Array.isArray(property.images)) {
            const existingImages = property.images.map((img) => ({
                type: 'existing',
                id: img.id,
                url: '/storage/property_images/' + img.path,
                name: img.alt_text,
            }));
    ...
    const handleDrop = (newFiles) => {
        ...
        setPropertyImages((prev) => [...prev, ...formatted]);
    };

    const handleRemove = (indexToRemove) => {
        const removed = propertyImages[indexToRemove];
    ...
    const appendFormData = () => {
        ...
        propertyImages.forEach((item) => {
            if (item.type === 'new') {
                ...
                formData.append('property_images[]', item.file);
            } else if (item.type === 'existing') {
                // Append the existing image id or path so backend knows to keep it
                // For example:
  ...
  const handleUpdatePropertySubmit = (e) => {
    e.preventDefault();
    const formData = appendFormData();

    router.post(route('property.update'), formData, {
      method: 'post',
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
